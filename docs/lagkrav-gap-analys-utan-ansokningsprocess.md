...
Datum: 2026-07-17
Scope: Bedömning av nuläge i Klinikklar för ledningssystem, avvikelser, riskanalyser och årshjul/egenkontroll. Ansökningsprocessen är medvetet exkluderad.
...
- Kvarstående gap finns i formellt fastställande, verifierad praktisk tillämpning, behörighetstester, spårbar effektuppföljning och juridisk detaljanpassning efter exakt verksamhetsform.
...
- Bevis på faktisk månatlig/kvartalsvis uppföljning i drift saknas.
...
Gap:
- Lex Maria/Lex Sarah-beslutsordning behöver formaliseras och tillämpas.
- Krav på verifierad effekt före stängning behöver kunna visas i verkliga ärenden.
- Eskaleringsvägar och ansvar behöver vara beslutade och utbildade i organisationen.
...
Gap:
- Enhetlig metodik och tröskelvärden för prioritering behöver beslutas och dokumenteras.
...
Gap:
- Obligatorisk resultatrad (utfall, avvikelse ja/nej, åtgärd, deadline, verifiering) behöver säkerställas i varje kontrollcykel.
- Ledningsmötesrutin för försenade kontroller behöver dokumenterad tillämpning.
...
- Rutiner för gallring, logggranskning och incidenthantering för personuppgifter behöver verifieras och dokumenteras.
- Behörighetsmatris och återkommande access review saknas i underlaget.
...
- Tydlig beviskedja vid tillsyn behöver sammanställas: identifierad brist -> åtgärd -> effekt.
...
- Medel:
  - Ojämt dokumenterad effektverifiering vid stängning av risk/avvikelse/kontroll.
...
5. Inför obligatorisk effektverifiering innan stängning i process och uppföljning.
- Leverabel: dokumenterad check med datum, ansvarig och verifierat utfall.

6. Upprätta och tillämpa behörighetsmatris + access review.
- Leverabel: matris per roll och återkommande granskning (ex. kvartalsvis).

7. Dokumentera dataskyddsrutiner för gallring, logggranskning och incidenthantering.
- Leverabel: beslutad rutin + genomförd första kontroll.

### P2 (mognad)

8. Etablera KPI-dashboard för compliance.
- Exempel: andel öppna avvikelser > 30 dagar, andel försenade kontroller, andel höga risker utan aktiv åtgärdsplan.

9. Genomför internrevision på kvartalsbasis.
...
- Processkedjan fungerar i praktik: avvikelse/risk/kontroll -> åtgärd -> verifierad effekt.
- Dataskydd och behörighetsstyrning är verifierade i återkommande kontroll.
